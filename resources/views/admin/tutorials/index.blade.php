                    ...
                    @forelse ($tutorials as $tutorial)
                        <tr class="hover:bg-indigo-50 transition-colors duration-300">
                            <td class="px-6 py-3 whitespace-nowrap">
                                @if($tutorial->videoId)
                                    <a href="{{ $tutorial->youtube_url }}" target="_blank" class="inline-block rounded-lg overflow-hidden shadow-md hover:shadow-xl transition duration-300">
                                        <img src="https://img.youtube.com/vi/{{ $tutorial->videoId }}/hqdefault.jpg" alt="Thumbnail {{ e($tutorial->title) }}" class="w-40 h-24 object-cover hover:scale-105 transition-transform duration-300" />
                                    </a>
                                @else
                            ...
                            <td class="px-6 py-3 max-w-lg text-gray-700 text-sm truncate" title="{{ $tutorial->description }}">
                                {{ \Illuminate\Support\Str::limit($tutorial->description, 130) }}
                            </td>
                            <td class="px-6 py-3 whitespace-nowrap text-center space-x-3">
                                ...
                                <form action="{{ route('admin.tutorials.destroy', $tutorial) }}" method="POST" class="inline-block" onsubmit="return confirmHapus(event)">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
...
<script>
    ...
    function confirmHapus(e) {
        ...
        Swal.fire({
            ...
            text: "Tutorial ini akan dihapus secara permanen.",
            icon: 'warning',
